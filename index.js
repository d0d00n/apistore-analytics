...
app.get("/monitor", async (req,res) => {
    ...
    let resultDatasets = [];
    if (analyticsCache.get(startDate+endDate+frequency+unpublished)){
        resultDatasets = analyticsCache.get(startDate+endDate+frequency+unpublished);
    }
    else{
        var servicePromises = [];
        ...
        analyticsCache.set(startDate+endDate+frequency+unpublished, resultDatasets);
    }
...
app.get("/", async (req, res) => {
    ...
    let totalHits = 0;
    if (analyticsCache.get('totalHits'+startDate+endDate+frequency+unpublished)){
        totalHits = analyticsCache.get('totalHits'+startDate+endDate+frequency+unpublished);
    }
    else{
